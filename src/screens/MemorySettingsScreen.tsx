...
import { SafeAreaView, View, Text, StyleSheet, Switch, TextInput, TouchableOpacity, Alert, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function MemorySettingsScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 8 : 0} style={{ flex: 1 }}>
        <View style={styles.content}>
          ...
          <View style={styles.card}>
            <View style={styles.rowBetween}>
              ...
              <Switch value={clearOnExit} onValueChange={setClearOnExit} thumbColor={clearOnExit ? Colors.primary : Colors.divider} />
            </View>

            <View style={[styles.rowBetween, { marginTop: 16 }] }>
              <View style={{ flex: 1 }}>
                <Text style={styles.label}>Keep last X conversations</Text>
                <Text style={styles.hint}>Limits history length per assistant (0 = unlimited).</Text>
              </View>
              <TextInput
                style={styles.input}
          ...
          <TouchableOpacity style={[styles.saveBtn, saving && { opacity: 0.6 }]} onPress={onSave} disabled={saving} accessibilityLabel="Save memory settings">
            <Text style={styles.saveText}>Save Settings</Text>
          </TouchableOpacity>

          <View style={styles.dangerCard}>
            <Text style={styles.dangerTitle}>Danger Zone</Text>
            <TouchableOpacity style={styles.clearBtn} onPress={onClearAll} accessibilityLabel="Clear all memory">
...
const styles = StyleSheet.create({
  ...
  input: { width: 80, backgroundColor: Colors.divider, color: Colors.textPrimary, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 8, textAlign: 'center' },
  saveBtn: { marginTop: 16, backgroundColor: Colors.primary, borderRadius: 14, paddingVertical: 12, alignItems: 'center' },
  saveText: { color: Colors.background, fontWeight: '800' },
  dangerCard: { marginTop: 20, backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.divider, borderRadius: 14, padding: 16, alignItems: 'center' },
  dangerTitle: { color: Colors.textPrimary, fontWeight: '800', marginBottom: 8 },
  clearBtn: { borderWidth: 1, borderColor: '#DC2626', borderRadius: 12, paddingVertical: 10, paddingHorizontal: 16 },
